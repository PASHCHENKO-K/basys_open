...
const VERSION = "basys-394db806ac86cdf0";
...
const PRECACHE = [
  ...
  "/basys_open/app/_expo/static/js/web/entry-4fe326acd511a128f4af9a391ea9f46e.js",
...
self.addEventListener('install', (event) => {
  /*
    ...
    `skipWaiting()` не зовётся. Воркер умеет заменить себя немедленно — и тогда
    новая версия приезжает посреди работы. Правило «обновление не прилетает
    посреди подхода» действует и здесь: новый воркер ждёт, пока закроются все
    вкладки, то есть встаёт при следующем открытии — ровно как бандл
    по воздуху на Android.
  */
